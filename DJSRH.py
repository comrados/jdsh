...
class DJSRH:
    ...
    def cal_similarity_matrix(self, F_I, txt):
        ...
        S_tilde = self.config.BETA * S_I + (1 - self.config.BETA) * S_T
        S = (1 - self.config.ETA) * S_tilde + self.config.ETA * S_tilde.mm(S_tilde.t()) / self.config.BATCH_SIZE
        S = S * self.config.MU
    ...
    def cal_loss(self, code_I, code_T, S):
        ...
        loss = self.config.LAMBDA1 * loss1 + 1 * loss2 + self.config.LAMBDA2 * loss3
